...
| spire-server.tornjak.config.tlsSecret | string | `"tornjak-server-tls"` | Name of the secret containing server side key and certificate for TLS verification (required for `tls` or `mtls` connectionType) |
...
| spire-server.tornjak.image.tag | string | `"v1.4.0"` | Overrides the image tag |
...
| spire-server.tornjak.service.portHttp | int | `10000` |  |
